...
| **04 Evaluation** | LLM-as-a-judge, **hit rate, MRR**, LLM-generated ground truth, structured outputs | Ground-truth generator, 4-arm retrieval eval, 3-prompt judge eval | **done** — retrieval: 4 arms × 235 questions; passage hit@5 **0.638** vs book hit@5 **0.906** for `hybrid_rerank` (ADR-001). LLM judge: ADR-003 historical **30**-question bake-off kept production (null result); committed archive table is **10 × 4** — see `EVAL.md`. **Added 2026-09-07:** book-level hit-rate, RRF `k` sweep (flat), answer–chunk cosine as semantic-overlap proxy, chunk-size experiment on **6/18** books / 73 scored — gap-report items 3, 4, 5, 8. Eval gate: committed-report CI step + `--from-run` (archive ≠ live bake-off) |
| **05 Monitoring** | Streamlit chat + dashboard, PostgreSQL, **Grafana**, token/cost tracking, 👍/👎 feedback | `query_log` + in-UI feedback + Grafana (Postgres) and Observatory ≥5 charts (SQLite tip path) | **done** — Observatory exposes **nine chart definitions** on the SQLite tip path (panels populate when telemetry exists) + thumbs feedback; Grafana (Postgres) is the v1 surface (ADR-005). OpenTelemetry stage tracing, list-price-equivalent cost from `LLM_PRICE_PER_1K_*` (or tokens-only when unpriced), operator-run online judge (`scripts/judge_recent.py`, off by default), demo answer cache — see [`zoomcamp-2026-gap-report.md`](zoomcamp-2026-gap-report.md) items 1, 2, 6, 9 |
...
every request. When `LLM_PRICE_PER_1K_*` are set, Observatory shows an
**estimated list-price equivalent** (not the amount Groq bills — free-tier may
remain $0). When prices are unset/`0`, the chart shows **token totals only**
(more honest than `$0.00`).
