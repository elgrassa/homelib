...
def answer_shelf_meta(
    ...
    *,
    ...
    if catalog_total is not None and catalog_total > 0:
        catalog_note = (
            f" The recommendation catalog has {catalog_total} Open Library works "
            "(metadata only — not full text on this shelf)."
        )
    ...
    if not shelf:
        ...
        if matched:
            ...
            body = (
                ...
                f"({len(matched)} of {len(shelf)} full-text titles):"
                f"{catalog_note}\n{lines}"
            )
        else:
            ...
            body = (
                ...
                f"or author. Here is what is on the shelf "
                f"({len(shelf)} full-text books):{catalog_note}\n"
                f"{available}"
            )
    else:
        lines = "\n".join(f"- {_format_book_line(b)}" for b in shelf)
